...
- **마일스톤 일자도 TBD가 아닙니다** — 2026-08-21 회의록으로 전체 확정됐습니다(미결정_대장
  ...
  둘입니다** — N19-a(재시도 횟수와 이음매별 배치, 이슈 #180), N23(`Accepted` ADR 의 본문 치환
  조건, PR #264 리뷰). **B-11 이 닫히면서 나온 확인 한 건은 2026-08-28 에 빠졌습니다** —
  08-24 회의가 이미 답했고(닫기는 진입 시점 `needsInput` 세션에만) 그 답을 대장에 반영했기
  때문입니다([회의록](docs/회의록/2026-08-28_잔여_6건_확정.md) 06). ⚠️ **결정이 반영된 것은
  문서이고 코드는 아직입니다** — `session_flow.apply_brief_patch` 는 진입 시점을 보지 않으며
  08-28 에 재현으로 확인했습니다(대장 B-11 소절 끝). 한때 이 목록에 있던 N21(브랜드
  스타일 일치도 행 처리)은 **2026-08-25 회의가 닫았고**, 08-26 에 N22 와 N23 이 붙었습니다.
  **N22(스텁에서 만화형을 눌렀을 때의 처리, 이슈 #271)는 2026-08-27 에 닫혔습니다** —
  회의가 TL 에게 위임했고 [ADR-0020](docs/adr/0020-스텁도_만화형_여섯_칸을_채운다.md)이
  "스텁도 여섯 칸을 채운다"로 확정했습니다. 화면 게이트는 2026-08-28 에 걷혔으므로
  **만화형 시안 화면은 열려 있고 기본 설정(`stub`)에서도 됩니다.** 목록의 정본은
