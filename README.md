...
> ⚠️ **워킹 스켈레톤이 2026-08-19에 관통했고, 지금은 이음매 교체 단계입니다.** 브라우저에서
> 입력부터 결과 저장까지 한 번에 지나가며 `e2e/`의 브라우저·HTTP 하네스가 그것을 고정합니다.
> **관통했다는 것과 실물로 돌았다는 것은 다릅니다** — 기본값이 `ADGEN_GENERATION_MODE=stub`
> 이고, `image:render` 만 실물 분기가 배선됐으며([ADR-0003](docs/adr/0003-이미지_생성_경로.md))
> `brief:fill` 과 `draft:generate` 는 비어 있습니다. **이 상태의 품질 숫자는 자기 자신과의
