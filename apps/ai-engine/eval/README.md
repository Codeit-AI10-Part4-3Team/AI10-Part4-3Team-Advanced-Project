...
같은 PR에서 함께 움직여야 측정이 재현되기 때문입니다([ADR-0001](../../../docs/adr/0001-monorepo.md)).
...
- **가드레일 on/off 델타가 핵심 숫자입니다.** `suppression_rate(off, on)` — off는 대조군이므로
  "테스트를 통과시키려고" 끄면 지표 자체가 무효가 됩니다.
