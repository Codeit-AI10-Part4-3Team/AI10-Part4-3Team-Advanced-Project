...
- **이미지 생성 경로는 TBD가 아닙니다** — 외부 API(`gpt-image-2`)로 확정됐고
  ...
  ([ADR-0017](docs/adr/0017-만화형은_칸을_따로_생성해_합성한다.md)). 열려 있는 것은
  **로컬 모델 채택 여부** 하나이고 판단 근거는 검증 4순위 결과입니다. 채택하면 base 모델
  라이선스 원문 확인이 선행되며, **하드웨어 예산은 이미 확정**입니다(가용 VRAM **22.5GB** ·
  호스트 RAM 16GB · 디스크 100GB, 2026-08-10 실측) — 넘는 후보는 검토 대상이 아닙니다:
...
- 품질 지표 목표치. **브랜드 데이터셋 출처와 권리 범위는 보류**입니다 — 파인튜닝을 하지 않으므로
  ([ADR-0004](docs/adr/0004-파인튜닝_보류.md)) 학습 데이터가 없고, 로컬 모델 채택 시 되살아납니다
