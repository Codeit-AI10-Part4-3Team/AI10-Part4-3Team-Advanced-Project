...
- **광고 생성 경로가 backend 에서 서빙됩니다** (2026-08-14, S2 ~ S6). 세션이 `created` 에서
  `completed` 까지 계약만 밟고 가며, 렌더는 backend 프로세스 안의 배경 작업이 실행합니다
  ([ADR-0015](docs/adr/0015-렌더_워커는_같은_프로세스의_배경_작업으로_돌린다.md)).
  ⚠️ **그래도 이것은 여전히 스켈레톤입니다.** `image:render` 의 실물 분기는 2026-08-15 에
  외부 이미지 생성 API 로 배선됐지만(ADR-0003), **기본값은 `ADGEN_GENERATION_MODE=stub`**
  이고 그 상태에서 나오는 이미지는 측정값이 아닙니다. `brief:fill` 과 `draft:generate` 의
  실물 분기는 아직 비어 있습니다 — 즉 지금 실물로 켤 수 있는 이음매는 렌더 하나뿐입니다.
- 템플릿의 질의응답(`/v1/ask` → `/v1/generate`)은 아직 **남아 있습니다.** 광고 경로가 그 옆에
  생긴 것이 아니라, 이음매(`AiEngineClient`)를 공유하며 갈아끼우는 중입니다. 지울 시점은
  프론트가 광고 경로로 옮겨간 뒤이고, 그때까지 이 경로는 인증이 없습니다
  ([API_계약.md](docs/기술문서/API_계약.md) 7.1절). 이음매 목록:
