...
>
> 같은 회의록이 "VM 스켈레톤 배포 완료"도 기록했지만 **그 항목은 아직 완료가 아닙니다.**
> 무엇이 완료된 것인지는 5-a절에 있습니다.
...
**PM이 08-14로 확인했습니다** (2026-08-12, PR #69 리뷰). 아래 gantt와 마일스톤 표가 이미 그
값이므로 날짜 변경은 없습니다. 다만 이 확인은 PR 코멘트이고, 대장 항목의 확정 근거는 회의록뿐입니다
(기획서 14.3). [미결정_대장.md](../기술문서/미결정_대장.md) B-20은 여전히 미확정입니다.

**2026-08-13 결정 이후:** 스켈레톤 관통이 08-17로 당겨지면서 그 실질 마감(08-14)이 중간 점검일과
같은 날이 되었습니다. 두 마일스톤이 사실상 한 지점이 되었으므로, 아래 3-b가 답을 받기 전에는
08-14에 "중간 점검"과 "관통 판정"으로 각각 무엇을 보여야 하는지가 사람마다 다릅니다.

### 3-b. 중간 점검의 판정 기준이 성립하지 않습니다 - 대기
...
**아직 답을 받지 못했습니다.** PR #69 리뷰에서 확인된 것은 3-a(날짜)뿐이고 판정 기준은 그대로
대기입니다. 기준 없이 08-14를 맞으면 점검에서 무엇을 보여야 하는지가 사람마다 달라집니다.
...
**범위를 줄이는 결정은 이 문서에 없습니다.** 2026-08-13 회의는 날짜만 정했고 범위 축소는 다루지
않았습니다. 08-14에 관통이 서지 않으면 날짜를 임의로 미루지 말고 7절의 축소 순서를 적용하고,
무엇을 뺐는지 [구현_범위.md](../공통_가이드/구현_범위.md)에 남기세요.
...
| 스켈레톤 관통 | **2026-08-17** | 브라우저에서 입력부터 파일 다운로드까지 한 번에 통과 (구현_범위 1절 전체 DoD). 08-17은 휴지기이므로 **실질 마감은 08-14** |
...
| 3 | **스택 배포** - compose로 backend와 ai-engine을 기동, 외부에서 8000 응답 | [착수_체크리스트.md](../공통_가이드/착수_체크리스트.md) 5-a | **미완료** (목표 08-14) |
...
08-12 실측은 **8000의 방화벽 규칙은 열려 있으나 밖에서 재면 응답이 없다**는 것입니다
(05 임동규). 막힌 것이 아니라 듣고 있는 프로세스가 없다는 뜻이고, 스택이 아직 올라가지
않았다는 근거입니다.

**후속: 회의록의 해당 줄은 다음 회의에서 정정합니다.** 정정 전까지 이 문서는 3번을 미완료로
둡니다. 완료로 적으면 05의 완료 조건("외부에서 접근 가능")이 충족되지 않은 채 닫히고,
아직 관통하지도 않은 스켈레톤이 VM에서 돌고 있다는 뜻이 되기 때문입니다.

**3번의 목표일은 08-14입니다** (2026-08-13, TL 신호정). 원래 05 일정과 gantt는 08-20이었고
[착수_체크리스트.md](../공통_가이드/착수_체크리스트.md) 5-a와 `infra/README.md`는 "W2 중"이어서
같은 작업이 두 날짜로 적혀 있었습니다(05 임동규 발견, PR #71). 이제 세 문서가 08-14로 같습니다.
**TL 초안이므로 확정 근거는 여전히 회의록입니다.**
...
- **계약 교체는 2026-08-12에 끝났습니다.** 이것이 막고 있던 S1(계약 골격)도 함께 충족됐습니다 —
  세션·시안·확정·잡 경로가 스펙에 있고 `contracts-check`가 통과합니다. **지금 앞을 막고 있는 것은
  더미 구현 3종**이며, 그것이 없으면 05의 S3·S4 호출부를 검증할 수 없습니다.
- **검증 1순위와 2순위가 이음매 교체의 선행 조건입니다.** 대사 처리 방식이 뒤집히면
  `image:render`를 무엇으로 채울지가 달라집니다. 스켈레톤은 이 결과를 기다리지 않지만
  교체는 기다립니다.
- **배포를 마지막 주로 미루지 마세요.** VM 환경 설정은 끝났지만 **스택은 아직 올라가지
  않았습니다**(5-a절). 08-14에 빈 스택이라도 VM에 한 번 올려 두면 "우리 환경에서만 안 되는 것"을
  미리 발견합니다.
