...
#
# ⚠️ **이 스크립트 자신을 고치는 배포는 한 번 실패합니다.** bash 는 실행 시작 시점의 파일을
#    붙들고 끝까지 돌므로, `update_checkout` 이 이 파일을 새 판으로 갈아도 **도는 것은 옛
#    판**입니다. 2026-08-28 에 실제로 났습니다 — 옛 판의 `show_changes` 가 `grep && warn` 으로
#    끝나 함수 반환값이 1 이 됐고(#292), 그것을 고친 #295 가 **같은 배포에 실려 있었는데도**
#    적용되지 않았습니다.
#    **복구는 재실행입니다** — 체크아웃은 이미 새 커밋이라 `prev == now` 로 조기 반환하고,
#    그때는 새 판을 읽습니다. 실패 지점이 `bring_up` 전이면 이미지는 옛것이므로 **반드시 다시
#    돌리세요** — 그 상태가 이 스크립트가 `--no-build` 와 `--ref` 조합을 거부하면서까지
#    막으려던 "HEAD 와 도는 이미지가 다르다" 입니다.
#    실측: docs/보고서/배포_관통_실패모드_실측_보고서.md A-b절
#
