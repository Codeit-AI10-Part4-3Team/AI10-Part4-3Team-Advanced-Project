...
- **뜬 직후 `chmod 600`으로 좁힙니다.** 파일 안에 `users.password_hash`와 세션이 통째로
...
- ⚠️ **볼륨에 파일을 넣을 때 `cp` 도 bind mount 도 쓰지 않습니다.** 파일을 stdin 으로
  ...
  `attempt to write a readonly database` 로 죽습니다), bind mount 는 0600 으로 좁혀 둔
  백업을 컨테이너가 읽지 못합니다. 둘 다 **복원은 성공한 것처럼 보이고 스택만 안 뜹니다**
  (2026-08-19 실측).
