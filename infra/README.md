...
`Via: 1.1 Caddy` 는 남깁니다 (RFC 9110 이 전달하는 프록시에 요구합니다). HSTS 는 넣지
...
> ⚠️ HTTPS 확인은 `curl --resolve`로 **loopback에 붙여서** 합니다. 확인이 DNS와 자기 외부
> IP로의 되돌기(헤어핀)에 기대지 않게 하기 위해서입니다 — 둘 다 배포의 성질이 아니라 그날
> 네트워크의 성질이라, 기대면 스택은 멀쩡한데 확인만 실패하는 경우가 생깁니다. SNI와 Host에는
> 진짜 이름이 실리므로 인증서 검증을 건너뛰는 것이 아닙니다 — 발급이 실패했으면 여기서 걸립니다.
>
> **2026-08-21 실측: 이 VM은 헤어핀이 됩니다** (VM 안에서 공개 호스트로 직접 재니 `200`).
> 그 전까지 이 자리에는 "GCP VM은 자기 외부 IP로 되돌아오지 못한다"고 적혀 있었으나 이
> 환경에서는 사실이 아닙니다. `--resolve`를 유지하는 근거는 위와 같이 바뀌었습니다.
...
- **뜨기 전에 롤백 저널을 회수합니다.** 백업은 `--user "$(id -u)"` 로 도는데 그 사용자는 uid
  10001 소유의 상태 파일을 **읽기로만** 열 수 있습니다. 저널이 남아 있으면 SQLite 가 읽기 전에
  복구를 하려 하고, 복구는 쓰기라 `attempt to write a readonly database` 로 끊깁니다 — 권한
  문제인데 손상과 구별되지 않는 메시지이고, 하필 **크래시 루프에서 사전 백업을 뜨려는 상황**이
  정확히 저널이 남는 상황입니다. 그래서 `--user` 없이(= 이미지 기본 사용자 appuser 로) 한 번
  여는 단계를 앞에 둡니다. 여는 것만으로 회수가 끝나고, 저널이 없으면 아무 일도 하지 않습니다
  (PR #133 리뷰 N3, 신호정. 2026-08-22 에 VM `/tmp` 에서 재현하고 수정을 확인했습니다 —
  단, 페이지가 실제로 DB 파일로 흘러넘친 뒤 죽은 경우에만 재현되며 경계는 재지 않았습니다).
