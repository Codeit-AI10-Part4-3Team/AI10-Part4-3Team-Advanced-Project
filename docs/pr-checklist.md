...
9. PR 서식([템플릿](../.github/pull_request_template.md))에 **무엇을 했는지 / 어떻게 동작을 확인하는지**를 기술하고,
   ...
   - ℹ️ 루트 `CODEOWNERS` 기준으로 리뷰어가 **자동 배정**됩니다. 변경한 경로의 코드오너 승인 1건이
     있어야 머지되며, **본인이 오너인 경로라도 자기 PR은 스스로 승인할 수 없습니다** — 같은 경로의
     다른 오너(대개 Tech Lead)에게 리뷰를 요청하세요.

10. 제출 후 **팀 메신저로 리뷰어에게 PR 요청**을 알립니다.
    - 📝 메시지 서식은 [메신저_규약.md](공통_가이드/메신저_규약.md)를 따릅니다.
      **첫 줄에 무엇을/누가/언제까지, 상세는 PR 링크 뒤로, 5줄 이내**입니다.
      배경 설명을 메시지에 붙이면 PR 본문과 두 벌이 되고 이틀 뒤 둘이 다른 말을 합니다.
...
11. 리뷰어가 PR을 확인하고, 수정이 필요한 부분에 **댓글로 피드백**을 남깁니다.
    - 📝 코멘트 서식은 [코드_리뷰_가이드.md](공통_가이드/코드_리뷰_가이드.md)를 따릅니다.
      **결론이 담긴 제목 + 처리 등급**(`머지 전 필수` / `이 PR에서 권장` / `후속 과제` /
      `참고, 조치 불필요`)을 반드시 붙이세요 — 등급이 없으면 작성자가 무엇이 머지를 막는지
      알 수 없어 모든 코멘트를 필수처럼 처리하게 됩니다.
    - ⚠️ 스타일·포맷·import 정렬·타입은 지적하지 마세요. ruff·mypy·reviewdog가 이미 판정합니다.
