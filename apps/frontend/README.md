...
| 무엇 | 어디서 이어지나 |
| --- | --- |
| 브리프 · 시안 부분 수정 (`PATCH`) | F5 (06 일정 08-21). 지금은 `filledBy` 표시까지만 |
| `brief_filling` 세션 진행 | 위와 같습니다. 지금은 새 세션이 유일한 길이고 화면이 그렇게 말합니다 |
...
| `/sessions/:sessionId` | 브리프 확인 → 시안 → 확정 → 렌더 상태 → 결과 | 로그인 필요 | **있음** (시안 수정은 F5) |
...
- 정보 부족·열화 상태의 추가 입력 흐름 — **미완.** 표시까지만 하고 추가 입력은 F5
- 브리프 잠금 전 확인 모달 — 모달 대신 버튼 위 경고 문구로 두었습니다
- 시안의 허용 필드만 인라인 수정 — **미완.** F5
