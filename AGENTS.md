...
- **대사 길이 상한 수치와 초과 시 처리** (미결정_대장 N18) — 안전 확인은 15자 이하, 관측된
  실패는 30자이고 **16 ~ 23자와 25 ~ 29자에 표본이 없습니다**. 세는 규칙은 `len()`(공백과
  문장부호 포함) — 집행하는 쪽이 `draft:generate`의 검사 코드이기 때문입니다. 소관 03
