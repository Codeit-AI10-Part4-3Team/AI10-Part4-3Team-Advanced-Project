...
⚠️ **이 파일이 스택을 가로지르는 유일한 테스트입니다.** `test_harness.py` 는 `/health` 만
확인하며, 예전에 그 파일이 지나던 템플릿 질의응답(`/v1/ask`)은 삭제되었습니다
(API_계약.md 7절). 이 파일이 없으면 "종단 관통 테스트 통과"가 관통을 뜻하지 않는 상태가 되고,
그것이 `AGENTS.md` 가 말하는 "게이트 통과 != 관통 경로 검증"입니다.
