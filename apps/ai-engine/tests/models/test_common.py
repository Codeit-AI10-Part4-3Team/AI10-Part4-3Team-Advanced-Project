...
Deliberately a near-copy of the same file in apps/backend. The two apps must not import
each other (AGENTS.md), so the commons are duplicated — and duplicated rules are only safe
if both copies are held to the contract independently. If this file ever starts importing
the backend's, the boundary is already gone.
"""
...
from ai_engine.models import Base, Error, ErrorCode, Omittable, OutputType
...
def test_error_code_matches_the_contract(contract_schemas: dict[str, Any]) -> None:
    """The whole enum, not just the four codes this app emits — see models/common.py."""
