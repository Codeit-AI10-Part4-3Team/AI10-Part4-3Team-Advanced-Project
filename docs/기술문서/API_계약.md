...
| `PATCH .../brief` 재입력 후에도 불확실 | 422 `INSUFFICIENT_INPUT` | `failed` | 아니요. 새 세션입니다. **2026-08-22 확정, 구현됨** ([회의록](../회의록/2026-08-22_이월_6건_확정.md) 03절, B-11). **다만 2026-08-24 회의가 좁힌 조건(진입 시점에 `needsInput` 이 걸린 세션에만 닫음)은 코드에 미반영이며 알려진 한계입니다** - 아래 2.1절 |
| `PATCH .../brief` 재추론이 돌지 못함 | 200 + `messageMode: degraded` | `brief_filling` | 예. 사용자가 두 값을 직접 채우면 `brief_ready` (2026-08-20). **주의: 조건부입니다** - 두 값을 채우기 전에 메모만 고치면 재추론이 돌고, 그것이 판단 불능이면 세션이 닫힙니다. 08-24 회의가 닫지 않기로 좁힌 자리이고 미반영이라 **알려진 한계**입니다(2.1절) |
...
**`image:render` 요청에는 업로드 사진의 바이트가 실립니다** (2026-08-29,
[ADR-0022](../adr/0022-제품_사진을_1번_칸의_레퍼런스로_보낸다.md)). 필드는
`ImageRenderRequest.productImage`(base64)이고 **선택 항목**입니다 - 사진 보존이 24시간이라
([세션_보관_정책.md](세션_보관_정책.md) 2절) 뒤늦게 도는 렌더에는 사진이 없으며, 그때는 키를
생략합니다(`null` 은 이 계약에 없습니다. 3절). 없다고 실패시키지 않습니다.

`brief.productImageUrl` 로 대신할 수 없습니다. 그 경로는 backend 인증 뒤에 있고 ai-engine 이
그것을 부르는 순간 의존 방향이 뒤집힙니다([AGENTS.md](../../AGENTS.md) 아키텍처 경계). 바이트가
요청에 실리지 않으면 렌더 시점의 모델은 제품을 한 번도 본 적이 없고 제품명만 보고 포장을
지어냅니다 - 2026-08-29 실물 회차 19세션 중 아홉 건이 그 증상이었습니다.
...
**주의: 열화로 시작한 세션도 첫 판단 불능에서 닫힙니다. 이것은 확정된 규칙이 아니라 알려진
한계입니다** (아래 문단). 세션을 만들 때 엔진이 죽어 있던 사용자는 `needsInput` 을 한 번도
받지 못하고 "카테고리와 타겟을 직접 채우라" 는 안내만 봅니다.
그 사람이 두 값을 채우기 전에 **메모만 고치면** 그 patch 가 재추론을 부르고(2.1절 조건 셋을
모두 만족합니다), 그 재추론이 판단 불능이면 거기서 세션이 닫힙니다. 되묻는 화면을 본 적이
없으므로 사용자 입장에서는 "한 번의 기회" 를 쓴 적이 없고, 원래 안내받은 탈출구도 함께
사라집니다. 같은 patch 에서 `category` 만 채우고 `target` 을 못 채운 경우도 같습니다 -
살아남는 것은 **둘 다** 채운 경우뿐입니다.

2026-08-22 회의록 03절은 닫는 조건을 "모델이 응답했으나 브리프를 채우지 못한 경우" 로만
적었고 되물음을 받았는지는 조건에 없어, 위 동작이 그 확정 문구에는 어긋나지 않았습니다. 이
경로가 회의에서 검토되지 않았을 가능성이 있어 미결정_대장 B-11 에 후속으로 적어 두었고
(PR #204 리뷰, 신호정), **2026-08-24 회의가 그 후속을 좁혔습니다** - 결론은 "닫기는 진입
시점에 `needsInput` 이 걸려 있는 세션에만 적용한다" 입니다
([회의록](../회의록/2026-08-24_제출_전_8건_확정.md) 안건 05).

**그 결정은 코드에 반영되지 않았습니다** - 2026-08-28 에 재현으로 확인했습니다.
`session_flow.apply_brief_patch` 가 세션을 닫는 조건에 진입 시점의 `needsInput` 유무가 들어
있지 않아, 열화로 시작해 되물음을 한 번도 받지 못한 세션이 첫 판단 불능에서 곧바로 422
`INSUFFICIENT_INPUT` 으로 닫힙니다. **따라서 위에 적은 동작은 지금 실제로 도는 동작이며,
확정된 규칙이 아니라 제출 시점의 알려진 한계로 읽어야 합니다.** 대장 표기도 "08-24 결정
미반영(재현됨)" 입니다 ([미결정_대장.md](미결정_대장.md) B-11, 근거는
[2026-08-28 회의록](../회의록/2026-08-28_잔여_6건_확정.md) 안건 06).

**화면이 종료를 알 수 있는 순간은 422 응답 하나뿐입니다.** 그 응답에는 세션이 실리지 않고
`revision` 은 이미 올라가 있으므로, 같은 patch 를 그대로 재시도하면 `409 REVISION_CONFLICT`
("최신 세션을 다시 읽고 시도하세요") 가 오고, 다시 읽고 보내면 `409 STATE_CONFLICT`
("세션이 `failed` 상태입니다") 가 옵니다. 둘 다 종료를 설명하지 않습니다. 두 경우 모두 엔진을
부르지 않으므로 닫힌 세션이 비용을 더 쓰지는 않습니다 (PR #204 리뷰에서 신호정 실측).
...
**주의: 계약 밖에 정적 자산 경로가 하나 생겼고, 그것은 인증 밖입니다** (2026-08-22).
`/static/art-styles` 가 화풍 예시 8장을 내줍니다. 위 문단과 어긋나 보이지만 다른 축입니다 -
`/v1` 의 예외가 아니라 `/v1` 이 아닌 경로이고, `ArtStyle.exampleImageUrl` 이 형태 제약 없는
문자열이라 서빙 방식이 계약 밖에 있습니다.

`StaticFiles` 에는 라우트 의존성이 걸리지 않아 인증 뒤에 둘 방법이 없습니다. 라우트로 만들면
그것은 `/v1` 즉 계약 경로가 되고(갈래 B), 그러면 이 절의 규칙이 그대로 적용됩니다. 여덟 장이
사용자 데이터가 아니라 우리가 만든 제품 자산이라 받아들인 교환입니다.

**이 예외도 새 경로로 물려주지 마세요.** 자산이 아니라 데이터를 내주는 순간 갈래 B 로 가야
합니다. 그 디렉토리가 상태 파일(`db_path`, `image_dir`, `log_dir`)을 함께 열어 주지 않는지는
backend 가 기동에서 검사하고, 어기면 뜨지 않습니다.
...
| 항목 | 결정 | 근거 |
|---|---|---|
| 동기 단계 타임아웃 상한 | 30초 (`brief:fill`) / 60초 (`draft:*`), 설정값 | 2절, [2026-08-29 회의록](../회의록/2026-08-29_브리프_타임아웃_완화.md) 01 |
...
**이 경로는 화면이 쓰는 것입니다.** ai-engine 은 여기를 부르지 않습니다 - 인증 뒤에 있어
부르는 순간 의존 방향이 뒤집힙니다. 그림에 실물 제품을 반영하는 길은 조회가 아니라 **바이트를
렌더 요청에 싣는 것**이며(1.2절,
[ADR-0022](../adr/0022-제품_사진을_1번_칸의_레퍼런스로_보낸다.md)), 두 경로는 같은 사진의 서로
다른 용도입니다.
