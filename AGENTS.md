...
> 🧩 **이 파일은 템플릿 상태입니다.** `TODO`로 표시된 절을 프로젝트 내용으로 채우세요.
> `TODO`가 남아 있는 동안 에이전트는 프로젝트를 이해하지 못한 채 코드를 씁니다.
> 반대로 **"아키텍처 규칙"·"함정"·"빌드/실행/테스트" 절은 이미 코드가 강제하고 있는 사실**이므로
...
- **누구의 어떤 문제를 푸는가**: TODO
- **핵심 마일스톤**: TODO (절대 날짜로)
- **저장소 공개 여부**: TODO — public이라면 커밋되는 모든 것이 공개된다고 전제하세요.
...
## 설계 제약 (TODO — 편의상 "단순화"하면 안 되는 것)

> 프로젝트의 하드 제약을 여기에 적으세요. **왜**를 함께 적지 않으면 규칙은 무시됩니다.
> 아래 둘은 템플릿이 코드로 강제하고 있는 제약입니다.

- **근거 기반 생성.** 생성 계층은 검색된 원문에만 근거해 답합니다. 가드레일의 on/off 델타 자체가
  측정 지표이므로, 우회하면 문제가 해결되는 게 아니라 지표가 무효가 됩니다.
- **열화 폴백 필수.** 외부 의존이 죽으면 조용히 실패하는 대신 사전 승인된 응답으로 전환되고,
  그 사실이 응답(`messageMode`)에 드러나야 합니다.
...
`CODEOWNERS`·`docs/역할_가이드/`·이 표는 **같은 이름·같은 경로**여야 합니다.
...
| 폴더 | 담는 것 |
|---|---|
| `docs/공통_가이드/` | 전원이 읽는 설계 문서 — `개발자_가이드.md`, `환경_세팅_가이드.md`, `저장소_운영.md`, `아키텍처.md`, `리스크.md`, `구현_범위.md` |
...
- 실험 → `notebooks/` (import 금지. 검증된 로직은 `apps/<app>/src/`로 승격)
