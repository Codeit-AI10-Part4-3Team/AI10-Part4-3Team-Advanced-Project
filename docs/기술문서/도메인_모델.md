...
- **`productImage`라는 이름은 이제 두 자리에 있습니다** (2026-08-29,
  [ADR-0022](../adr/0022-제품_사진을_1번_칸의_레퍼런스로_보낸다.md)). 공개 계약의 업로드
  파트(multipart 파일)와, 내부 계약의 `ImageRenderRequest.productImage`(같은 사진의 base64
  바이트)입니다. 뒤의 것은 **브리프 필드가 아니라서** 위 표에 없습니다 - 저장된 사진을 backend가
  렌더 호출 때 읽어 싣는 값이고, 사진이 만료되면 키 자체가 사라집니다. 브리프에 남는
  `productImageUrl`은 그대로 참조이며 엔진은 그것을 부르지 않습니다
  ([API_계약.md](API_계약.md) 1.2절).
...
- 시안의 모든 텍스트는 `sellingPoint`, `note`, `productName` **셋**에 근거해야 합니다. 근거 없는
  수치와 효능은 가드레일이 걸러냅니다 (범위의 정본은
  [생성_파이프라인.md](생성_파이프라인.md) 5.2절). 제품명이 근거에 들어가는 것은 범위를 넓히려는
  것이 아니라 **사용자가 직접 친 글자를 우리가 지어낸 것으로 세지 않기 위해서**입니다.
...
| INV-6 | 생성물의 사실 주장 범위 <= `sellingPoint` + `note` + `productName` ([생성_파이프라인.md](생성_파이프라인.md) 5.2절) | 가드레일 검사 + 대조군 측정 | 표시광고법 위반 (5.5, 9.3) |
