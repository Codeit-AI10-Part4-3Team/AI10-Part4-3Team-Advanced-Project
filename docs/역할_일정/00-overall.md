...
> **상태: TL 재산정안입니다 (2026-08-21 기준 재작성).** 만드는 방법은 2026-08-11 회의가 정한
> 그대로입니다 - AI로 초안을 뽑고 이의가 있으면 수정합니다. **확정은 회의록으로 합니다**
> ([미결정_대장.md](../기술문서/미결정_대장.md) B-20). **2026-08-21 확정 회의가 이 안을
> 검토**하며, 확정 회의록이 커밋되면 이 안내와 각 역할 문서의 "TL 초안" 안내를 지웁니다.
> 그 전에는 아래 날짜를 지연 판정의 근거로 쓰지 마세요.
>
> **예외 - 협의 대상이 아닌 값 둘.** "스켈레톤 관통" 일자(2026-08-17)는
> [2026-08-13 회의록](../회의록/2026-08-13_워킹스켈레톤_일정_조정.md)으로 확정되었습니다.
> **제출 마감 2026-08-30은 학원 공지(2026-08-21)로 당겨진 외부 확정값**입니다 - 회의가 정할
> 수 있는 것은 그 앞의 배치이지 이 날짜가 아닙니다.
>
...
2. **스켈레톤 이후 구간을 이제 계획할 수 있게 되었습니다.** 08-11 판은 관통까지만 유효했고
   ...
   기준, D2(가드레일 델타) 측정 방식 - 은 **08-21 확정 회의에서 일정과 함께** 다룹니다 (4-b절).
...
gantt
    ...
    section 마일스톤
    일정과 미결정 4건 확정 :crit, milestone, m1, 2026-08-21, 0d
    ...
    section 검증 트랙
    3순위 판정 회수       :crit, v1, 2026-08-21, 2d
    화풍 예시 8종 생성    :crit, v2, 2026-08-22, 3d
    D2 대조 재실행       :v3, 2026-08-24, 2d
    5순위 회차와 판정     :crit, v4, 2026-08-25, 2d
    가드레일 대조 판정    :v5, 2026-08-26, 1d

    section 기능 트랙
    B4 대사 상한 분기     :f1, 2026-08-22, 1d
...
|---|---|---|---|
| W3 잔여 | 08-21 ~ 08-22 | 2일 | 일정과 미결정 4건이 **회의록으로 확정**(B-20 닫힘). 검증 3순위 판정 회수와 집계. B4(대사 상한) 분기 구현. 화풍 예시 생성 착수. F3 완료 |
...
| 전제 | 걸려 있는 것 | 어긋나면 |
|---|---|---|
| 08-21 회의가 미결정 4건을 닫음 | B4 구현(08-22), D2 재실행(08-24 ~ 25), 3순위 판정 확정 | 해당 항목이 밀리는 것이 아니라 축소 대상이 됩니다. 회의 없이 코드가 먼저 갈 수 없는 항목들입니다 (미결정_대장 등급 규칙) |
...
| **일정과 미결정 4건 확정** | **2026-08-21** | 확정 회의록이 커밋되어 B-20이 닫히고, 이 문서와 역할 일정의 "TL 초안" 안내가 제거됨. 함께 다루는 4건: N18 상한, 타임아웃 값, 장면 상태 연속성 기준, D2 측정 방식 |
...
1, 2, 4, 6, 8순위가 닫혔고 3순위는 판정 배치 12세트가 조립되어 판정자 회신만 남았습니다.
**크리티컬 패스는 화풍 예시 8종입니다** - 검증 5순위와 F2 화면과 `ADGEN_ART_STYLES` 반영이
전부 그 뒤에 서 있고, 늦으면 마지막 사흘의 실측 구간을 침범합니다.

- **회의가 첫 마디입니다.** B4(대사 상한)와 D2(가드레일 델타)와 3순위 판정 확정이 전부 08-21
  회의록에 걸려 있습니다. 회의록 없이 코드가 먼저 갈 수 없는 항목들입니다 (미결정_대장 등급
  규칙 - `차단`만 코드를 막고 `잠정 진행`은 진행).
