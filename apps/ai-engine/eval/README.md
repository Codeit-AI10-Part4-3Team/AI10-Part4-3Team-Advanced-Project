...
**회차 기록에는 가드레일이 검사한 문구가 `texts`로 함께 남습니다**(2026-08-25). `trap`
케이스는 `check_claims`가 못 잡는 갈래라 사람이 회차 출력을 직접 봐야 판정되는데, 본문을
버리면 그 판정을 하려고 같은 호출을 다시 사야 합니다. `adPlan`과 `visualPlan`은 넣지
않습니다 — 소비자에게 하는 주장이 아니라 제작 지시문이고, 가드레일이 검사하는 것도
`texts`에 담긴 문자열뿐이라 판정 대상이 어긋납니다(ADR-0019). 실측 결과의 정본은
[지표_실측_보고서.md](../../../docs/보고서/지표_실측_보고서.md)입니다.
