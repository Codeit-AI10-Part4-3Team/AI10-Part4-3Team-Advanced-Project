...
| `draft:generate` / `draft:patch` | 60초 |
  ...
  **두 프록시의 상한은 모두 앱 상한보다 커야 합니다.** 프록시가 먼저 끊으면 클라이언트가
  ...
  (`apps/frontend/nginx.conf`), Caddy는 응답 타임아웃 기본값이 무제한이라 값을 적지 않은
  상태가 이 불변식을 만족합니다. 이 불변식은 프록시를 무엇으로 바꾸든 성립합니다.
...
| 호스트에 열리는 포트 | 프록시의 80과 443 둘뿐입니다. backend 8000은 발행하지 않습니다 |
...
**8000을 발행하지 않기로 한 것은 계약 확인 경로에 영향이 있습니다.** `curl`로 계약을 직접
두드리던 주소가 프록시 주소로 바뀝니다. 열어 두지 않는 이유는 로그인이 비밀번호를 보내는데
([ADR-0008](../adr/0008-로그인_수단과_스켈레톤_범위.md)) 그 문을 평문으로 남기면 HTTPS를
붙이는 의미가 없어지기 때문입니다. FastAPI의 `/docs`는 프록시가 넘기지 않으므로 스키마를 볼
때는 uvicorn을 직접 띄우세요.
