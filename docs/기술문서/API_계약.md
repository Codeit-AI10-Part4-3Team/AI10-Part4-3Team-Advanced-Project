...
| `PATCH .../brief` 재입력 후에도 불확실 | 422 `INSUFFICIENT_INPUT` | `failed` | 아니요. 새 세션입니다. **잠정안이고 아직 구현되지 않았습니다** - 아래 2.1절. 확정은 18.2 #11 |
...
- 재입력 후에도 판단이 서지 않을 때의 처리는 미정입니다 (기획서 18.2 #11). 그때까지는
  `INSUFFICIENT_INPUT` 오류로 끊는 것을 잠정안으로 둡니다. **아직 구현되지 않았습니다.**
...
`PATCH .../brief` 가 엔진을 다시 부르는 조건은 **둘 다** 참일 때입니다.

| 조건 | 왜 |
|---|---|
| 세션에 `needsInput` 이 있음 | 계약이 재시도를 약속한 것은 이 상태뿐입니다. `degraded` 세션에는 다른 출구가 이미 있습니다(사용자가 두 값을 직접 채움) |
| patch 가 `productName`, `sellingPoint`, `note` 중 하나 이상을 건드림 | 이 셋이 `brief:fill` 이 읽는 전부입니다. `artStyle` 만 바꾼 요청에 재추론을 걸면 같은 답을 유료로 다시 사는 것입니다 |
...
**주의: 세 번째 행이 열린 자리입니다.** 계약은 여기서 422 `INSUFFICIENT_INPUT` 과 `failed` 를
약속하지만 그렇게 하지 않습니다. **몇 번 만에 포기하는가**가 미결정_대장 B-11 이고 확정 근거가
회의록이라, 코드가 숫자를 먼저 정할 수 없기 때문입니다. 그때까지 이 경우는 제자리에 남습니다 -
되묻는 횟수가 무한하다는 뜻이고, 그것이 B-11 이 여는 위험 그대로입니다.
