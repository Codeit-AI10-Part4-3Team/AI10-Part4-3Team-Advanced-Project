...
둘 다 회의 몫입니다. 근거 규칙상 확정은 회의록만 할 수 있습니다(기획서 14.3).

- **만화형 화면 개방** (#271). 스텁에서 눌렀을 때의 처리가 미결정이라 게이트가 닫혀 있습니다.
  엔진과 backend 의 만화형 실물 경로는 08-26 에 관통이 확인됐으므로
  ([만화형 실물 검증 보고서](../보고서/만화형_실물_검증_보고서.md)) 남은 것은 화면 몫뿐입니다.
  **정해지지 않으면 7절 축소 순서 3번이 이미 답입니다** - 시연은 단일 광고형입니다. 이 문서가
  새로 정하는 것이 아니라 확정된 축소 순서를 가리키는 것입니다.
