...
| `brief` | 객체 | 3절. **`draft_generating` 진입 이후 변경 불가** (7절 INV-7) |
| `briefMeta` | 객체 | 4절. `brief`와 같은 시점에 잠깁니다 |
| `draft` | 객체 또는 `null` | 5절. `draft_ready` 이전에는 `null`. **`finalized` 이후 변경 불가** (INV-2) |
...
- **고칠 수 있는 구간은 `brief_ready`까지입니다.** 시안 생성을 요청하는 순간 브리프 전체가
  잠기고, 그 뒤로는 고치는 경로가 없습니다 (7절 INV-7). 아래 표의 `editable`은 **잠기기 전**의
  이야기입니다.
- `outputType`은 브리프에 없습니다. 세션에 하나만 둡니다 (2절).
...
| INV-7 | `draft_generating` 진입 이후 `brief` 변경 불가 | 상태 기계 가드 -> 409 `STATE_CONFLICT` | 시안이 무엇에 근거했는지 알 수 없게 됩니다 |
...
| INV-7, INV-8 | **미작성** | 기획서 9.2는 브리프를 "확정 전까지 수정 가능"으로 두었고 `adPlan`도 시안의 일부였습니다. 둘 다 그보다 이르게 잠그는 쪽으로 뒤집힌 결정이라 근거가 남아야 합니다 (PR #39 리뷰) |

INV-7과 INV-8의 ADR은 아직 없습니다. 근거가 PR 리뷰 코멘트에만 있는 상태이고, 이것은
[AGENTS.md](../../AGENTS.md) "결정의 근거"가 허용하는 참조 범위이되 **결정 기록으로는 부족합니다**.
다음 PR에서 ADR로 옮깁니다.
...
| 톤 항목 (유머형 / 감동형 등) 존치 | 스키마에 없습니다. PM 의견은 **미존치**이며, `adPlan`이 서술형 문단이 되면서 담을 자리도 사라졌습니다. 항목 자체는 열려 있습니다 (18.2 #9) |
| 선택 필드를 비웠을 때의 표현 (`null` / 키 생략) | 미정. 3절의 "적용되지 않는 유형에서는 필드 자체가 없다"와는 다른 문제입니다 |
