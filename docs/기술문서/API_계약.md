...
| `PATCH .../brief` 재입력 후에도 불확실 | 422 `INSUFFICIENT_INPUT` | `failed` | 아니요. 새 세션입니다. **2026-08-22 에 확정**(B-11, 판단 불능 1회) 되었고 **구현은 아직입니다** - 아래 2.1절 |
...
- 재입력 후에도 판단이 서지 않을 때의 처리가 **2026-08-22 에 확정되었습니다**
  ([회의록](../회의록/2026-08-22_이월_6건_확정.md) 03, B-11). 판단 불능은 **1회**까지이며
  그 1회도 실패하면 `INSUFFICIENT_INPUT` 으로 세션을 닫습니다. **아직 구현되지 않았습니다.**
...
| 재입력 후 | `PATCH .../brief` | **422** `INSUFFICIENT_INPUT` | `failed` (2026-08-22 확정, 구현 대기) |

`failed`는 [용어_사전.md](용어_사전.md) 3.1절 상태도의 `brief_filling -> failed` 간선입니다.
**되돌아오는 간선이 없으므로 그 세션은 끝나고 새 세션을 만들어야 합니다.** 18.2 #11 은
2026-08-22 에 이 표와 같은 값으로 확정되었으므로 표와 상태도를 고칠 것이 없습니다.
...
| 여전히 판단 못 함 | 새 `reason` 으로 교체 | `normal` | `brief_filling` **(열린 자리)** |
...
**주의: 세 번째 행은 아직 구현되지 않았습니다.** 계약은 여기서 422 `INSUFFICIENT_INPUT` 과
`failed` 를 약속하지만 코드는 그렇게 하지 않습니다. **숫자가 2026-08-22 에 정해졌으므로**
([회의록](../회의록/2026-08-22_이월_6건_확정.md) 03, B-11) 남은 것은 구현뿐입니다 - 판단 불능
**1회**이며, 그 1회도 실패하면 세션을 닫습니다. 구현될 때까지 이 경우는 제자리에 남고, 되묻는
횟수가 무한하다는 뜻입니다.

**엔진 다운은 이 횟수에 넣지 않습니다.** 응답 자체가 없는 것은 판단이 서지 않은 것과 다르며,
지금처럼 `degraded` 로 빠져 세션을 닫지 않습니다. 둘을 같은 횟수로 세면 벤더 장애 한 번에
사용자가 재입력 기회를 영영 잃습니다.
