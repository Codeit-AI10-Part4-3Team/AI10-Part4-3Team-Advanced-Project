...
| 3 | 캐릭터와 화풍 일관성 | 판정 완료 (2026-08-21). 별도 문서: [검증3순위_일관성_판정_보고서.md](검증3순위_일관성_판정_보고서.md) |
| 4 | 로컬 모델과 GPT 비교 | GPT 측 기준선 확보 |
| 5 | 화풍 반영 | 이 시험에서는 착수 불가 (선행 조건 미확정). 이후 판정 완료 (2026-08-24). 별도 문서: [검증5순위_화풍반영_판정_보고서.md](검증5순위_화풍반영_판정_보고서.md) |
...
| 1 | `1팩 80매.` | 숫자 |
| 2 | `리필팩 NEW` | 영어 혼용 |
| 3 | `무향 무알코올이라 아이 손을 닦아도 안심할 수 있어요.` | 긴 문장 (30자) |
...
### E-b-2. 운영 경로에서 같은 값이 나왔습니다 (2026-08-20)

위 수치는 전부 실험 하네스에서 나온 것입니다. 하네스가 통과했다는 것과 서비스가 통과했다는
것은 다르므로, 컷별 생성이 `ai-engine`에 구현된 뒤
(`POST /v1/image:render`, [ADR-0017](../adr/0017-만화형은_칸을_따로_생성해_합성한다.md))
같은 시나리오로 두 회차를 다시 측정했습니다. 프롬프트와 대사는 1순위와 같습니다.

| 티어 | 응답 시간 | 캔버스 | 칸 크기 | 바깥 여백 | 대사 오탈자 |
|---|---|---|---|---|---|
| `low` | 52.8초 | 3456 x 2304 | 1152 x 1152 정확 | 0px | 0 / 6 |
| `medium` | 118.0초 | 3456 x 2304 | 1152 x 1152 정확 | 0px | 0 / 6 |

**하네스 값과 같은 구간입니다.** `low`는 55.9초 대 52.8초, `medium`은 139.2초 대 118.0초로
둘 다 하네스보다 빠르게 나왔습니다. 호출자의 `render_timeout_s`(300초) 안에 듭니다.

칸 크기는 계산이 아니라 실측입니다. 여섯 칸이 모두 요청한 1152 x 1152 그대로 돌아왔습니다.
회차 당시 구현은 크기가 어긋나면 경고를 남기고 늘려 붙이도록 되어 있었는데 경고가 한 번도
나오지 않았습니다. 지금은 그 자리가 경고가 아니라 세트 실패입니다 -- 늘려 붙이면 규격 위반이
합성물에서는 통과로 보이기 때문입니다 (PR #150 리뷰 반영). 바깥 여백은 캔버스 테두리에 합성
배경색이 한 픽셀도 드러나지 않는 것으로 확인했습니다.

![운영 경로 medium 회차](API생성_시험_예시_이미지/컷별생성_운영경로_medium.jpg)

**새로 관측된 것 둘입니다.**

- **칸 경계선의 굵기는 여전히 회차마다 다릅니다.** 우리는 경계선을 그리지 않지만 모델이 칸
  안쪽에 자기 테두리를 그립니다. 이음매의 어두운 폭이 `low` 10px, `medium` 1px이었습니다.
  칸 크기와 여백은 합성이 결정하므로 고정 좌표 크롭은 그대로 성립하고, 흔들리는 것은 잘라낸
  칸이 테두리를 얼마나 두껍게 물고 나오는지입니다.
- **장면 상태 연속성 문제가 운영 경로에서도 재현되었습니다.** `medium` 회차의 6번 칸에서
  1번 칸의 쏟아진 커피와 넘어진 컵이 다시 나타났습니다. 4번과 5번 칸에서는 깨끗한 상태였습니다.
  인물과 복장은 여섯 칸 내내 유지되었으므로, 1번 칸 레퍼런스가 잡는 축과 못 잡는 축이 갈린다는
  2026-08-20 판정 결과와 같습니다 ([미결정_대장.md](../기술문서/미결정_대장.md) A-4).

주의: 티어당 1세트입니다. 그리고 이 두 회차는 판정자 교차 검증을 거치지 않았습니다 -- 재려던
것이 규격과 시간이고, 그 둘은 사람 판정이 아니라 픽셀과 시계로 재는 값이기 때문입니다.
...
2. **긴 문장에서는 오탈자가 발생합니다.** 30자 문장에서 조사 오류가 관측되었습니다.
