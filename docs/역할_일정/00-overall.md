...
이 정정 자체는 PR 코멘트였지만, 대장 항목의 확정 근거인 회의록이 2026-08-21에 나왔습니다
([일정 확정 회의록](../회의록/2026-08-21_일정_확정.md)). [미결정_대장.md](../기술문서/미결정_대장.md)
B-20도 그날 닫혔습니다.
...
| 06이 화면 6건과 판정 참여를 함께 소화 | F1, F2, F3, F5, 대기 화면, 열화 표기, F10 + 판정 3회 | 7절 1 ~ 2순위(Grid Post 확인, F10 세분화)를 먼저 버립니다 |
| 만화형 `medium` 예산 약 48세트 | 5순위 회차, 시연과 제출 산출물 | 검증과 개발은 `low`, 최종 확인만 `medium` (03 WORKLIST 0절 규칙 유지). 예산이 좁아지면 회차 수를 줄이지 티어를 올리지 않습니다 |

**2026-08-27: 위 표와 4-a절 간트에서 F13(면책 고지)을 뺐습니다.** F13은 2026-08-22 회의가
1차 범위에서 제외한 항목인데 두 자리가 그대로 남아 **06의 남은 작업 부하를 실제보다 많게
세고 있었습니다**. 근거는 [2026-08-27 회의록](../회의록/2026-08-27_안건_12건_확정.md) 13번
안건(이슈 #247)이고, 같은 회의 05번 안건이 문구만 확정하되 화면 반영은 이번 라운드 대상이
아님을 다시 확인했습니다. 7절 축소 대상 목록에는 애초에 F13이 없어 그쪽은 손대지 않았습니다.
...
flowchart LR
    ...
    classDef done fill:#e3f2e3,stroke:#2e7d32,stroke-width:1px
    class MEASURE,SUBMIT crit
    class MEET,V3,STYLES,F2V5,DEPLOY done
```

**날짜는 그대로이고 색만 바뀌었습니다.** 초록은 2026-08-26 현재 닫힌 노드, 빨강은 아직 남은
노드입니다. 날짜를 바꾸려면 회의록이 필요합니다(머리말).

**현재 상태는 6-b 입니다.** 6-a 는 08-21 재산정 시점의 기록이며 그때의 판단 근거로만 읽습니다.

### 6-a. 2026-08-21 기준 (이력)
...
### 6-b. 2026-08-26 기준 (이번 판)

**크리티컬 패스가 07 의 판정 회수로 옮겨졌습니다.** 6-a 가 크리티컬 패스로 지목한 화풍 예시
8종은 닫혔고, 그 뒤에 서 있던 것들도 함께 닫혔습니다. 날짜는 하나도 바뀌지 않았습니다.

**주의: 닫힌 날과 그것이 표에 도달한 날이 다릅니다.** 아래 다섯 중 셋(3순위 판정, 화풍 예시
8종, 배포와 재부팅 검증)이 **08-22 에 닫혀 있었고**, 화풍 예시는 그 사실이 확인되기까지 나흘이
걸렸습니다. 크리티컬 패스가 이미 지나간 뒤에도 문서가 그것을 병목으로 가리키고 있었다는
뜻입니다. 남은 사흘에는 같은 지연을 감당할 여유가 없으므로, 닫은 사람이 그날 소관 표를
고치는 편이 낫습니다.
...
| 노드 | 상태 | 근거 |
|---|---|---|
| 확정 회의 (08-21) | 완료 | [일정 확정](../회의록/2026-08-21_일정_확정.md), [미결정 5건](../회의록/2026-08-21_미결정_5건_확정.md) |
| 3순위 판정 회수 (08-22) | 완료 | [검증3순위 판정 보고서](../보고서/검증3순위_일관성_판정_보고서.md) |
| 화풍 예시 8종 (08-24) | **완료 08-22, 이틀 당김** (확인은 08-26) | [만화형 실물 검증 보고서](../보고서/만화형_실물_검증_보고서.md) C-c절. 파일 여덟 장의 `Last-Modified` 가 08-22 08:54 UTC 로 6초 안에 붙어 있습니다. 확인 내용은 8종 전부 각 1152 x 1152, 화풍 선택 격자 8칸 실렌더, 자리표시 0건 |
| F2 화면 + 5순위 (08-26) | 완료 | 화면은 08-21(#179, [06 표](06-프론트엔드.md)), 판정은 08-24([검증5순위 판정 보고서](../보고서/검증5순위_화풍반영_판정_보고서.md)) |
| 배포와 재부팅 검증 (08-24 ~ 25) | **완료 08-22, 이틀 당김** | [05 표](05-백엔드_인프라.md). 재부팅 후 개입 없이 14초 만에 4개 healthy, 백업 cron 실행까지 확인 |

#### 남은 노드

**지표 실측(08-26 ~ 28)에서 03 의 집계는 끝났습니다** -
[지표 실측 보고서](../보고서/지표_실측_보고서.md)와
[가드레일 대조 실측 보고서](../보고서/가드레일_대조_실측_보고서.md)로 남아 있습니다.
**남은 것은 07 의 독립 확인 세 건**이고, 그것이 지금의 크리티컬 패스입니다
([07 표](07-QA_보안.md)).

| 남은 일 | 목표 | 착수 조건 |
|---|---|---|
| 가드레일 대조군 판정 | 08-26 | **충족됨.** 막고 있던 #224, #225 가 둘 다 머지됐습니다 |
| 지표 실측 확인 | 08-28 | 충족됨 |
| E2E 관통 확인 | 08-28 | 구현은 05, 통과 판정은 함께 |
| 시크릿 점검, 제출 전 문서 점검 | 08-29 | 위 셋 뒤 |

**"결과를 만든 사람이 채점하지 않는다" 가 이 구간의 성질입니다**(구현_범위 4.4절). 그래서
집계가 끝났다는 것과 판정이 끝났다는 것이 다르고, 남은 사흘의 병목이 여기입니다.

#### 이 문서가 정하지 않는 것 하나

회의 몫입니다. 근거 규칙상 확정은 회의록만 할 수 있습니다(기획서 14.3).

- **컷 역할 재현 여부** (#272). 실물 모드 창(08-26 ~ 28)과 잔여 작업 예산(약 2.5 USD) 안에서만
  가능하고, 같은 창과 같은 지갑을 위 판정 회수가 함께 씁니다.

> **만화형 화면 개방(#271)은 이 목록에서 빠졌습니다.** 2026-08-27 회의(안건 02)가 TL 에게
> 위임했고 [ADR-0020](../adr/0020-스텁도_만화형_여섯_칸을_채운다.md)이 "스텁도 여섯 칸을
> 채운다"로 닫았습니다(미결정_대장 N22). 게이트는 2026-08-28 에 걷혔습니다.
> **7절 축소 순서 3번(만화형 화면 개방)을 발동할 이유가 없어졌습니다** - 시연에 만화형을
> 넣을 수 있습니다.

#### 크리티컬 패스는 아니지만 시연에 걸리는 것

**#274** - 배포된 Caddy 가 옛 설정으로 돌아 `nosniff` 가 빠져 있습니다. 화풍 예시가 지금
그려지는 것이 그 덕이라, **제출 전 재배포가 한 번이라도 있으면 F2 격자 8칸이 빈 칸이 됩니다.**
노드는 아니지만 시연 산출물에 직결됩니다.

#### 소관 표 갱신 대상

이 절이 닫힌 것으로 세는데 소관 표에는 아직 이전 상태로 남아 있습니다. 소관자가 갱신합니다.

- [05 표](05-백엔드_인프라.md)의 `ADGEN_ART_STYLES` 반영 배포 행 - "2단계는 08-24 전달 대기"
- [07 표](07-QA_보안.md)의 가드레일 대조군 판정 행 - "#224/#225 병합 후 착수 예정"
...
>
> **위 표의 3번도 2026-08-28 에 소진됐습니다.** 화면 몫이던 게이트가 걷혔고
> (N22 / [ADR-0020](../adr/0020-스텁도_만화형_여섯_칸을_채운다.md)), 기본 설정(`stub`)에서도
> 만화형 시안이 그려집니다. **행을 지우지 않는 것은 축소 순서 자체가 회의가 정한 값이기
> 때문이고**, 발동할 상황이 남아 있지 않다는 뜻입니다.
