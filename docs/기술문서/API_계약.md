...
| `PATCH .../brief` 재추론이 돌지 못함 | 200 + `messageMode: degraded` | `brief_filling` | 예. 사용자가 두 값을 직접 채우면 `brief_ready` (2026-08-20). **주의: 조건부입니다** - 두 값을 채우기 전에 메모만 고치면 재추론이 돌고, 그것이 판단 불능이면 세션이 닫힙니다(2.1절) |
...
**주의: 열화로 시작한 세션도 첫 판단 불능에서 닫힙니다.** 세션을 만들 때 엔진이 죽어 있던
사용자는 `needsInput` 을 한 번도 받지 못하고 "카테고리와 타겟을 직접 채우라" 는 안내만 봅니다.
...
적었고 되물음을 받았는지는 조건에 없으므로, 위 동작은 확정된 규칙 그대로입니다. 다만 이 경로가
회의에서 검토되지 않았을 가능성이 있어 **미결정_대장 B-11 에 후속으로 적어 두었습니다**
(PR #204 리뷰, 신호정).
