...
**여기까지가 실제 백엔드를 부르는 전부입니다.** 스튜디오 화면의 제출 버튼은 여전히 입력 내용을
오른쪽 확인 패널에 표시할 뿐이고, 세션 목록도 `mock-data.ts`입니다. 업로드, 실제 생성, 폴링,
다운로드는 아래 순서에 따라 연결합니다.
...
| --- | --- | --- | --- |
| `/login` | 로그인 | 공개 | **있음** |
| `/` | 새 광고 세션 생성 | 로그인 필요 | **있음** (아직 mock 데이터) |
| `/sessions/:sessionId` | 브리프 확인 → 시안 수정 → 렌더 상태 → 결과 | 로그인 필요 | 없음 |
...
> ⚠️ **배포 시 SPA 폴백이 필요합니다.** `BrowserRouter`를 쓰므로 `/login`을 주소창에 직접 치면
> 서버가 그 경로의 파일을 찾습니다. Vite 개발 서버는 알아서 `index.html`을 돌려주지만 정적
> 호스팅은 그렇지 않습니다 -- 새로고침 한 번에 404가 뜹니다. 정적 파일을 서빙할 주체가
> 정해질 때 rewrite 규칙을 함께 넣으세요 (`infra/docker-compose.yml`에는 아직 frontend
> 서비스가 없습니다).
