...
- **대사 길이 상한 수치와 초과 시 처리** (미결정_대장 N18) — **운영 조건(컷별 생성,
  `medium`)에서 25자까지 오탈자가 관측되지 않았습니다**(2026-08-20 판정 완료). 상한을 25자로
  두는 것을 실험이 막지 않지만 근거는 "25자까지 실패가 없었다"이지 "25자가 한계다"가
  아닙니다 — **26자 이상은 재지 않았습니다.** 관측된 실패 30자는 ADR-0017이 폐기한 한 장
  6칸 조건의 숫자라 그대로 옮기지 마세요. 세는 규칙은 `len()`(공백과 문장부호 포함) —
  집행하는 쪽이 `draft:generate`의 검사 코드이기 때문입니다. 소관 03
...
- ⚠️ **관통했다는 것과 실물로 돌았다는 것은 다릅니다.** 기본값은 여전히
  ...
  것이 아닙니다). **대사 길이 상한은 프롬프트에도 검사 코드에도 없습니다** (N18).
