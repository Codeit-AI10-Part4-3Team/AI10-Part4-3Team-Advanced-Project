...
주의: **`openapi.yaml`은 이 계약으로 교체되었지만 구현은 아직 템플릿의 질의응답 경로입니다.**
따라서 위 문장("스펙이 표현의 정본")은 지금부터 적용되고, 어긋나 있는 것은 문서와 스펙이 아니라
**스펙과 구현**입니다 -- 상세는 7절.
...
**이 문서와 `openapi.yaml`은 서로 다른 PR로 올라갔습니다.** 문서가 먼저이고 스펙 교체가 뒤입니다.
...
| 2 (**현재**) | 이 계약 | 이 계약 | 템플릿 질의응답 | 스펙(표현) + 이 문서(의도) |
| 3 | 이 계약 | 이 계약 | 이 계약 | 같음 |

**구간 2에서는 `openapi.yaml`을 근거로 프론트엔드와 클라이언트를 짜도 됩니다.** 거기 적힌 경로가
이 프로젝트의 계약입니다. 다만 **그 경로를 호출하면 아직 404입니다** -- 서버가 따라오지 않았기
때문이며, 화면 작업을 계약 기준으로 시작하되 관통 확인은 구현이 붙은 뒤에 하세요.
...
### 7.1 `/v1/ask`에는 인증을 걸지 않습니다 (2026-08-13 결정, 임동규)

6절의 보호 범위는 "`/health`와 `/v1/auth/*`를 뺀 모든 `/v1` 경로"입니다. 문자 그대로 읽으면
템플릿 잔재인 `/v1/ask`도 대상이지만, **걸지 않습니다.**

- 그 문장이 가리키는 것은 **이 계약이 정의한 경로**입니다. `/v1/ask`는 `openapi.yaml`에 없고
  위 표의 "구간 2"에서 교체를 기다리는 쪽입니다. 계약이 모르는 경로에 계약의 보호 규칙을
  적용하는 것은 규칙을 지키는 것이 아니라 범위를 넓히는 것입니다.
- **곧 지울 코드에 테스트를 늘리게 됩니다.** 인증을 걸면 기존 `/v1/ask` 테스트가 전부 쿠키를
  준비해야 하고, 그 준비 코드는 경로와 함께 지워집니다.
- 대신 **인증 의존성(`api.deps.current_user`)은 지금 만들어 두고 계약에 있는 경로에만 겁니다.**
  S2에서 세션 경로가 들어올 때 그대로 붙습니다.

인증 없이 열려 있는 동안의 노출 범위는 질의응답 하나이며, 이 경로는 계정, 세션, 브리프 어느
것도 읽거나 쓰지 않습니다.

**이 예외는 `/v1/ask`가 살아 있는 동안만 유효합니다.** 계약 경로로 갈아끼우는 순간 그 경로는
6절의 보호 범위에 그대로 들어갑니다 -- **예외를 새 경로로 물려주지 마세요.** 이 절은 그때
`/v1/ask` 삭제와 함께 지웁니다.
...
| HTTPS 종단 방식과 게이트웨이 상한 | 미정 (소관 05, 기한 배포 전). 프록시를 두면 그 상한은 앱 상한보다 커야 합니다 (2절). **다만 "HTTPS 를 둘지"는 미정이 아닙니다** - 아래 8.3절 |
...
`images.store` 가 지금 서버 파일 경로를 돌려주고 있고, 그 값이 `productImageUrl` 에 그대로
들어가 계약을 어기고 있었습니다. 경로가 생겼으므로 그 함수가 상대 경로를 돌려주도록 바꾸면
닫힙니다. 소관은 05 이며 이 문서의 변경과 같은 PR 이 아닙니다 - 계약이 먼저입니다
([AGENTS.md](../../AGENTS.md) 교체 순서).
