...
- **가드레일을 목으로 대체해 테스트를 통과시키지 마세요.** 우회한 가드레일은 테스트를 고치는
  게 아니라 측정을 무효로 만듭니다. 보고 지표는 **on/off 델타가 아니라 절대값**입니다
  (2026-08-21 회의) — 그래도 대조군 관측은 여전히 기록 대상이라, 켜 둔 채 "끔"으로 적으면
  관측 자체가 거짓이 됩니다. 정본: [생성_파이프라인.md](../../docs/기술문서/생성_파이프라인.md) 5.3절.
