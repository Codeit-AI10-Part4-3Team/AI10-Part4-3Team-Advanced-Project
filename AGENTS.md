...
- ⚠️ **관통했다는 것과 실물로 돌았다는 것은 다릅니다.** 기본값은 여전히
  ...
  ⚠️ **텍스트 세 이음매에 남은 두 구멍**은 각각 다른 결정에 묶여 있습니다.
  **가드레일은 프롬프트 쪽 절반만 붙었습니다** - 출력 검증(`ai_engine.guardrail.verify`)과
  1회 재생성이 아직 없어 `refusalReason: guardrail` 은 나올 수 없고, 지금 재는 on/off 델타는
  프롬프트 효과만 잽니다. **대사 길이 상한은 프롬프트에도 검사 코드에도 없습니다** (N18).
