...
VM에 그런 경로가 두 개 있고, **서로 다른 뿌리에 있습니다.**

```
/srv/adcraft/exp/   <- 공용 실험 폴더. 학습, 스크래치, 산출물 (이 절)
/app/               <- 배포 체크아웃. 서비스 스택이 여기서 뜹니다 (infra/README.md)
```

`exp`는 사람이 쌓는 작업물이고 `/app`은 `deploy-vm.sh`만 건드리는 트리입니다. 섞어 두면
디스크를 볼 때 "지워도 되는 것"과 "지우면 배포가 죽는 것"이 한 덩어리로 보입니다.
아래는 `exp` 쪽 설정입니다.
...
`/app`도 그룹·setgid·기본 ACL은 같은 방식이지만 **git 저장소라서 두 줄이 더 붙습니다**
(`core.sharedRepository`와 `safe.directory`). 그 절차와 이유는
[infra/README.md](../../infra/README.md)의 "배포 체크아웃은 `/app`" 절이 정본입니다.
여기에 복제하지 않습니다.
...
- **개인 홈의 체크아웃에서 배포하지 않습니다.** 배포 트리는 `/app` 하나입니다 (7-d절)
