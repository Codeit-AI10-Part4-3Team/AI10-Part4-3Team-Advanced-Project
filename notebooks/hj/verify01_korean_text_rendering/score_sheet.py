...
STYLE_DISTINCT_QUESTION = (
    # ⚠️ **어느 짝이 닮았는지 예를 들지 마세요.** 여기 "1번과 2번" 을 적어 두었었는데, 그것이
    #    바로 이 물음으로 확인하려던 짝입니다 - 지목하면 원하는 답이 그대로 돌아옵니다.
    #    C4 에서 그 지적이 값어치가 있었던 이유도 **묻지 않았는데 자유 응답에서 두 분이
    #    독립으로 적었기** 때문입니다. 아래 `3번과 7번` 은 형식을 보이는 예시일 뿐이라
    #    실제 후보가 아닌 짝을 골라 두었습니다.
    "## 구분되지 않는 화풍이 있습니까\n"
    "\n"
