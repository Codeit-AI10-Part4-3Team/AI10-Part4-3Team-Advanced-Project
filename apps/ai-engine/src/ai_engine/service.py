...
Keep routers thin: request validation -> engine call -> response mapping. Prompt building
and guardrail logic belong in sibling modules of this package, which must stay importable
without FastAPI so the eval harness can call them directly.

⚠️ **Every route here is in the contract now.** The template's `/v1/generate` (Q&A over a
fixture corpus) was deleted on 2026-08-20 together with `retrieval` and `generation` — it
was the last route this service served that `packages/contracts/openapi.yaml` did not
describe, and it survived only because it was the one route-level caller of
`guardrail.verify`. The ad path stopped needing that check in ADR-0019.
