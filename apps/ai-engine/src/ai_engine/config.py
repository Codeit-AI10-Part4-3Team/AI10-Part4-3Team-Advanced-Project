...
class Settings(BaseSettings):
    ...
    **핵심은 숫자가 아니라 누가 먼저 실패하는가입니다** (이슈 #141). 두 값이 같으면 어느 쪽이
    ...
    brief_fill_model_timeout_s: float = 25.0
    """`brief:fill` 이 벤더를 기다리는 상한. 호출자의 30초보다 **작아야 합니다.**

    ⚠️ **12 에서 올린 값입니다** (2026-08-29 실측). 12 로는 이 이음매가 거의 매번 예산을
    넘겼습니다 - 성공한 호출이 9,170ms, 예산에 걸린 호출이 12,018ms, 상향 뒤 되물음까지 간
    호출이 22,463ms 였습니다. 2026-08-26 회차의 13초 실측과도 맞물립니다. 사진을 함께 읽는
    유일한 이음매라 다른 텍스트 호출보다 느립니다.

    ⚠️ **이 값이 작으면 지표가 조용히 틀어집니다.** 죽은 것이 아니라 느렸을 뿐인데 호출자가
    열화로 빠지므로 `messageMode: degraded` 비율이 부풀고, 그 값이 보고 지표입니다.
