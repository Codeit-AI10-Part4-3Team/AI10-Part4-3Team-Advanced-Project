...
- **마일스톤 일자도 TBD가 아닙니다** — 2026-08-21 회의록으로 전체 확정됐습니다(미결정_대장
  ...
  셋입니다** — N19-a(재시도 횟수와 이음매별 배치, 이슈 #180),
  N27(만화형 1번 칸의 제품과 주배경, [ADR-0021](docs/adr/0021-만화형은_제품과_주배경을_1번_칸에_놓는다.md)),
  N28(업로드한 제품 사진을 렌더에 보내는가, [ADR-0022](docs/adr/0022-제품_사진을_1번_칸의_레퍼런스로_보낸다.md)).
  **뒤의 둘은 회의가 연 항목이 아니라 2026-08-29 실물 회차가 열었습니다** — 셋 다 잠정안대로
  코드가 돌고 있어 멈추는 작업은 없고, 두 ADR 은 회의가 확정하거나 권한을 위임해야
  `Accepted` 가 됩니다. 한때 이 목록에 있던 것들은
  전부 닫혔습니다 — N21(브랜드 스타일 일치도 행 처리)은 **2026-08-25 회의**, N23(`Accepted`
  ADR 의 본문 치환 조건)과 N26(사용자가 쓴 `note` 가 칸 역할의 금지를 이김, 이슈 #282)은
  **2026-08-27 회의**(안건 12 · 08), B-11 이 닫히면서 나온 확인 한 건은 **2026-08-28
  회의**(안건 06)가 닫았습니다. **N22(스텁에서 만화형을 눌렀을 때의 처리, 이슈 #271)만
  회의가 직접 닫지 않았습니다** — 2026-08-27 회의(안건 02)는 확정 권한을 TL 에게 위임했을
  뿐이고, 닫은 것은 아래 ADR-0020 입니다(위임 조건 충족은 2026-08-28 회의 안건 01 이
  확인). 목록의 정본은 [미결정_대장.md](docs/기술문서/미결정_대장.md)이며, 회의 이후에
  생긴 안건은 회의록에 없습니다.

  N22 를 닫은 것은 [ADR-0020](docs/adr/0020-스텁도_만화형_여섯_칸을_채운다.md)("스텁도 여섯
  칸을 채운다")이고, 화면 게이트는 2026-08-28 에 걷혔습니다 — **만화형 시안 화면은 열려 있고
  기본 설정(`stub`)에서도 됩니다.**
...
- ⚠️ **관통했다는 것과 실물로 돌았다는 것은 다릅니다.** 기본값은 여전히
  ...
  | 이음매 | 실물 분기 |
  |---|---|
  | `image:render` | 배선됨 (2026-08-15, ADR-0003). 만화형은 **칸을 따로 생성해 3x2 로 합성**합니다 (2026-08-20, ADR-0017). 1번 칸 뒤 다섯 칸은 동시 호출이고, 한 칸이라도 실패하면 세트 전체가 실패입니다. **업로드한 제품 사진이 요청에 실려 1번 칸과 단일 광고형의 레퍼런스가 됩니다** (2026-08-29, ADR-0022) - 사진이 없으면 그 전과 같은 그림을 그리며 실패가 아닙니다 |
