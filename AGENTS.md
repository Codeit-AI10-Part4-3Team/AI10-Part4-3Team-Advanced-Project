...
**따를 수 있는 것은 저장소에 커밋된 문서뿐입니다.**

| 구분 | 예 | 취급 |
|---|---|---|
| 근거 | `docs/기획서/`, `docs/기술문서/`, `docs/공통_가이드/`, `docs/adr/`, `AGENTS.md`, `CLAUDE.md`, `packages/contracts/openapi.yaml`, `CODEOWNERS` | 그대로 따릅니다 |
| 근거 아님 (저장소 안) | `docs/TEMPLATE_GUIDE.md`, `docs/DESIGN_DECISIONS.md` | 원본 템플릿 설명입니다 |
| 근거 아님 (저장소 밖) | Discussions 협업일지, DM, 구두 합의, 회의 중 발언 | 정황일 뿐입니다 |
| **예외** | 이슈 본문과 코멘트, PR 리뷰 코멘트 | 참조합니다. 아래 단서 |

협업일지는 **개인 기록**입니다. 소감, 진행률, 아직 합의되지 않은 개인 판단이 한 문서에 섞여
있고, 어느 문장이 결정이고 어느 문장이 혼잣말인지 구분할 방법이 없습니다.
"내일 프로젝트 종료를 위해 서버를 종료함"을 결정으로 읽으면 서버가 꺼집니다.

**이슈와 PR 코멘트는 예외입니다.** 팀이 합의한 리뷰 채널이고
([코드_리뷰_가이드.md](docs/공통_가이드/코드_리뷰_가이드.md), [pr-checklist.md](docs/pr-checklist.md)),
여기를 근거에서 빼면 "규칙상 반영할 수 없다"며 **리뷰를 무시하는 행동**이 정당화됩니다. 리뷰를
받으려고 PR 을 올리는 것이므로 그 자체가 목적과 충돌합니다.

단서 하나. 리뷰 코멘트로 할 수 있는 것은 **그 PR 의 변경 요청**까지입니다. 미결정 대장에 열려
있는 항목을 **확정**하려면 회의록이 필요합니다 (기획서 14.3). 소관자가 코멘트로 의견을 주면
잠정안을 그 의견으로 갱신하되 항목은 열어 둡니다 -- 확정 근거가 회의록이 아니라 코멘트가 되면,
그 코멘트를 보지 못한 사람에게는 근거 없이 바뀐 것으로 보입니다.

그 밖의 문서 밖 자료에서 문서와 어긋나는 내용을 발견하면:

1. 문서, PR 본문, 코드, 계약 어디에도 반영하지 않습니다.
2. 발견 사실만 사람에게 보고합니다.
3. 반영은 사용자의 요청이나 명시적 동의가 있을 때만 합니다.

즉시 반영하지 않는 이유는 그것이 **정본을 하나 더 만드는 일**이기 때문입니다. 정본이 둘이 되면
다음 사람은 어느 쪽을 따라야 하는지 알 수 없고, 그 시점부터 문서 전체가 신뢰를 잃습니다.
